...
| Phase-aware timing (S19) | `bridge::collect_round` uses three distinct timeouts gated by a `StreamToken::phase_boundary()` sentinel emitted by `runner` after `ensure_ready` succeeds: `LIFECYCLE_TIMEOUT=600s` (Phase 1 cold-load slack — ~2.4× the observed 248 s worst case on `qwen3-coder-next-200k:latest`), `TOKEN_FIRST_TIMEOUT=60s` (Phase 2 first token after load), `STREAM_IDLE_TIMEOUT=45s` (mid-stream idle). RoundError variants `LifecycleTimeout` / `FirstTokenTimeout` distinguish cold-stuck vs hung-post-load for retry decisions. `MCP_LIFECYCLE_PHASE=off` legacy path: no boundary emitted → bridge stays Phase 1 the whole round, single 600 s applies (strictly more permissive than the prior 240 s, no regression). SDD: `.specs/veronex/bridge-phase-aware-timing.md`. |
