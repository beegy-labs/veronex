...
execute_one(tool_call, api_key_id, allowed_servers)
  ...
  │     └── HTTP POST to MCP server /mcp  (JSON-RPC tools/call)
  │
...
| File | Purpose |
|------|---------|
| `infrastructure/outbound/mcp/bridge.rs` | `McpBridgeAdapter` — full loop impl |
...
| `veronex-mcp/src/` | MCP server implementation (weather) |
