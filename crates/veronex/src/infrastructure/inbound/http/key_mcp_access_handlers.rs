...
) -> Result<Json<McpAccessEntry>, AppError> {
    let key_uuid = parse_uuid(&key_id)?;
    let server_uuid: Uuid = body.server_id.parse()
        .map_err(|_| AppError::BadRequest("Invalid server_id".into()))?;
...
pub async fn revoke_key_mcp_access(
    RequireSettingsManage(_): RequireSettingsManage,
    State(state): State<AppState>,
    Path((key_id, server_id)): Path<(String, String)>,
) -> Result<StatusCode, AppError> {
    let key_uuid = parse_uuid(&key_id)?;
    let server_uuid: Uuid = server_id.parse()
        .map_err(|_| AppError::BadRequest("Invalid server_id".into()))?;
