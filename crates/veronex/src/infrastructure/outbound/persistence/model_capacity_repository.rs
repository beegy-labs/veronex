...
impl ModelCapacityRepository for PostgresModelCapacityRepository {
    ...
    async fn get(&self, provider_id: Uuid, model: &str) -> Result<Option<ModelCapacityEntry>> {
        let row = sqlx::query_as::<_, ModelCapacityRow>(
            "SELECT * FROM model_capacity WHERE provider_id = $1 AND model_name = $2",
        )
        .bind(provider_id)
        .bind(model)
    ...
    async fn list_all(&self) -> Result<Vec<ModelCapacityEntry>> {
        let rows = sqlx::query_as::<_, ModelCapacityRow>(
            "SELECT * FROM model_capacity ORDER BY provider_id, model_name",
        )
        .fetch_all(&self.pool)
        .await
