...
pub async fn submit_inference(
    ...
    let job_id = state
        .use_case
        .submit(&req.prompt, &req.model, &req.backend, Some(api_key.id), None, JobSource::Api, ApiFormat::VeronexNative, None, None, Some("/v1/inference".to_string()), None, Some(api_key.tier.clone()))
        .await
