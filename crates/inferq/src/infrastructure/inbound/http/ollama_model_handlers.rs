...
/// — list backends (id, name, url, status) that have the given model synced.
pub async fn list_model_backends(
    ...
    match state
        .ollama_model_repo
        .backends_info_for_model(&model_name)
        .await
    {
        Ok(backends) => {
            let dtos: Vec<OllamaBackendDto> = backends
                .into_iter()
                .map(|b| OllamaBackendDto {
...
/// — list model names synced for a specific Ollama backend.
pub async fn list_backend_models(
    State(state): State<AppState>,
    Path(backend_id): Path<Uuid>,
) -> impl IntoResponse {
    match state.ollama_model_repo.models_for_backend(backend_id).await {
        Ok(models) => {
    ...
    let backends = match state.backend_registry.list_all().await {
        Ok(all) => {
            let ollama: Vec<_> = all
                .into_iter()
                .filter(|b| b.is_active && b.backend_type == BackendType::Ollama)
                .collect();
            ollama
        }
        Err(e) => {
    ...
    tokio::spawn(async move {
        ...
        for backend in backends {
            ...
            let result = async {
                ...
                ollama_model_repo
                    .sync_backend_models(backend.id, &models)
                    .await?;
    ...
    match state.ollama_sync_job_repo.get_latest().await {
        Ok(Some(job)) => {
            let dto = OllamaSyncJobDto {
                ...
                status: job.status,
                total_backends: job.total_backends,
                done_backends: job.done_backends,
                results: job.results,
            };
