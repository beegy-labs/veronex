...
use crate::domain::enums::{ApiFormat, BackendType, JobSource};
...
async fn pick_ollama(state: &AppState) -> Result<LlmBackend, Response> {
    let backends = state
        .backend_registry
        .list_active()
        .await
    ...
    backends
        .into_iter()
        .find(|b| b.backend_type == BackendType::Ollama)
        .ok_or_else(|| {
            (
