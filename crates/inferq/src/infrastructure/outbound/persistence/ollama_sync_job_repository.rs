...
impl OllamaSyncJobRepository for PostgresOllamaSyncJobRepository {
    async fn create(&self, total_backends: i32) -> Result<Uuid> {
        ...
        sqlx::query!(
            r#"
            INSERT INTO ollama_sync_jobs (id, total_backends)
            VALUES ($1, $2)
            "#,
            id,
            total_backends,
    ...
    async fn update_progress(&self, id: Uuid, result: serde_json::Value) -> Result<()> {
        sqlx::query!(
            ...
            SET done_backends = done_backends + 1,
    ...
    async fn get_latest(&self) -> Result<Option<OllamaSyncJob>> {
        let row = sqlx::query!(
            r#"
            SELECT id, started_at, completed_at, status, total_backends, done_backends, results
        ...
        Ok(row.map(|r| OllamaSyncJob {
            ...
            status: r.status,
            total_backends: r.total_backends,
            done_backends: r.done_backends,
            results: r.results,
        }))
    }
}
