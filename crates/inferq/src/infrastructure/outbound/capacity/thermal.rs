...
/// Three-level thermal throttling state.
///
/// Transitions:
/// - Normal  → Soft  when temp ≥ 85°C
/// - Soft    → Hard  when temp ≥ 92°C
/// - Hard    → Soft  when temp < 85°C AND cooldown expired
/// - Soft    → Normal when temp < 78°C AND no cooldown active
///
/// Hysteresis at 78°C prevents rapid Normal↔Soft oscillation.
